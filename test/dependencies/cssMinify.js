...
module.exports = function (opts, isMinify) {
  ...
  return through.obj(function (file, enc, cb) {
    ...
    if (file.isStream()) {
      this.emit(
        'error',
        new gutil.PluginError('css minify', 'Streams are not supported!')
      )
      return cb()
    }
    if (isMinify) {
      var minimize = new CleanCSS(options).minify(file.contents.toString())
        .styles
      file.contents = new Buffer(minimize)
    }
    file.path &&
			options.pathReplace.forEach(item => {
